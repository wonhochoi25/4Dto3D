...
func resolve(id: int, time: float, result: Dictionary) -> Variant:
	...
	var local = entry.model.sample(time)
	if local == null:
		error_id = id
	...
	if not entry.model.is_group:
		var vertices: Array[Vector4] = []
		for point in entry.model.shape.vertices: vertices.append(Math4D.apply(world, point))
		points = local.projection.project(vertices)
		for point in points:
			if not point.is_finite():
				error_id = id
				error = "World projection overflow."
				return null
	result[id] = {"world": world, "points": points}
	return result[id]
