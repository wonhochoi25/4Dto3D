extends RefCounted
## Transform expressions only; usable without geometry, files, renderer or UI.
const MathExpression = preload("res://scripts/core/animation/math_expression.gd")
...
func _init(group: bool = false) -> void:
	is_group = group
...
## Initial in-memory settings are literal PRSA values, not compensated anchor edits.
...
func apply_sources(candidate: Dictionary, time: float) -> bool:
	var filtered := {}
	for key in sources: filtered[key] = str(candidate.get(key, sources[key]))
	candidate = filtered
	if is_group:
...
## Trial evaluation validates only the transform, not presentation.
func evaluate(time: float, programs: Dictionary = {}) -> Variant:
	return sample(time, programs)
