...
func remove_card(card) -> void:
	...
	if not graph.remove(group_id, timeline.time):
		show_tree_error(graph.error)
		return
	graph.entries.erase(geometry.get_meta("node_id"))
...
func display_time(value: float) -> bool:
	...
	if state == null:
		...
		if by_id.has(graph.error_id):
			...
			card.show_error(card.object.error_field, graph.error, true)
			var group_id: int = graph.error_id if card.object.is_group else card.get_meta("group_id")
			select_card(pairs[group_id].card)
			pairs[group_id].editor.current_tab = 1 if card.object.is_group else 0
		return false
	for id in by_id:
		var card = by_id[id]
		if card.runtime_error: card.clear_errors()
		if card.object.is_group: continue
		card.object.last_points = state[id].points
		card.renderer.render(card.object, state[id].points)
...
func reparent_node(id: int, parent: int) -> bool:
	timeline.pause()
	if not graph.reparent(id, parent, timeline.time):
		show_tree_error(graph.error)
		queue_tree_rebuild()
		return false
	tree_message.hide()
